...
DATA_SRC=""
for p in "/teamspace/studios/this_studio/Cham-OCR/ocr-training/data/cham_synthetic_v25" "./data/cham_synthetic_v25" "../data/cham_synthetic_v25"; do
    if [ -d "$p" ]; then
        DATA_SRC="$p"
        break
    fi
...
if [ -n "$DATA_SRC" ]; then
    ...
    echo "  - Sao chép dữ liệu huấn luyện từ $DATA_SRC vào /dev/shm..."
    ...
    echo "⚠️ Cảnh báo: Chưa tìm thấy thư mục cham_synthetic_v25!"
fi

# Xác định thư mục PaddleOCR
PADDLE_DIR="."
if [ -f "tools/train.py" ]; then
    PADDLE_DIR="."
elif [ -d "PaddleOCR" ]; then
    PADDLE_DIR="PaddleOCR"
elif [ -d "/teamspace/studios/this_studio/Cham-OCR/ocr-training/PaddleOCR" ]; then
    PADDLE_DIR="/teamspace/studios/this_studio/Cham-OCR/ocr-training/PaddleOCR"
fi
cd "$PADDLE_DIR"
echo "📂 Working directory: $(pwd)"

# Đảm bảo cấu hình và từ điển có mặt tại thư mục làm việc của PaddleOCR
mkdir -p data configs
cp -f ../data/cham_dict_v25.txt data/ 2>/dev/null || true
cp -f /teamspace/studios/this_studio/Cham-OCR/ocr-training/data/cham_dict_v25.txt data/ 2>/dev/null || true
cp -f ../configs/rec_cham_v25_lightning.yml configs/ 2>/dev/null || true
cp -f /teamspace/studios/this_studio/Cham-OCR/ocr-training/configs/rec_cham_v25_lightning.yml configs/ 2>/dev/null || true

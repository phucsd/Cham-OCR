...
def run_export():
    config_path = os.path.join(BASE_DIR, "ocr-training", "configs", "rec_cham_v24.yml")
    ckpt_path = os.path.join(BASE_DIR, "ocr-training", "output", "v24", "extracted_v24", "best_checkpoint", "best_accuracy")
    save_dir = os.path.join(BASE_DIR, "ocr-studio", "data", "output", "rec_cham_inference_v24")
    dict_path = os.path.join(BASE_DIR, "ocr-training", "data", "cham_dict_v24.txt")
    ...
    config["Global"]["export_with_pir"] = False  # Force legacy .pdmodel format via OldIrGuard!

    print(f"Exporting model with export_with_pir=False to {save_dir}...")
    export(config)

    print("\n✅ Export completed successfully! Listing files in save_dir:")
